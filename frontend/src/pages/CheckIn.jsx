...
export default function CheckIn() {
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        ...
        const res = await axios.get(`http://localhost:5000/api/v1/checkin/history?userId=${uid}&_t=${Date.now()}`);
  ...
  const logCopingAction = async (intent, strategy) => {
    try {
      await axios.post('http://localhost:5000/api/v1/checkin/action/coping', {
